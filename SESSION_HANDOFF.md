...
green. **Correction to carry forward: the check count is not fixed, so do not treat any
number as the bar.** The finish chain expected "14/14"; #257 ran **13** and the follow-up docs PR
#258 ran **14** — the difference is the `Auto-assign and Label PR` job, which did not fire on
#257. Verify that every check GitHub actually lists is green, rather than counting to a target. The next block of v2 redesign work is
...
| PR #257 checks | **13/13 pass** — 13 is what this PR ran, not a fixed repo-wide count (see Summary) | `gh pr checks 257` |
...
| **RETIRED: any fixed check count as the acceptance bar** | Measured: #257 ran **13** checks, the docs PR #258 ran **14**; the extra one is `Auto-assign and Label PR`, which did not fire on #257. The count varies per PR, so "all listed checks green" is the only durable bar. | Low — a session could stall waiting for a check that never runs, or stop one short |
