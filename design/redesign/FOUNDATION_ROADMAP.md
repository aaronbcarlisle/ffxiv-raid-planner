...
> **Status:** In execution · updated 2026-06-28 · integration base `redesign/foundation` (head `ee03eab`)
> **Progress:** **F0 ✅ · F1 ✅ · F2 ✅ · F3 ✅ — all merged into `redesign/foundation`** (F0 #157, F1 #155, F2 #158 `2ffde63`, F3 #159 `ee03eab`). **F4 (frontend structure) is NEXT.** Each phase = a branch off `redesign/foundation` → PR → squash-merge into foundation; `foundation → main` lands everything at the end. Release notes are at **v2.0.2** (F3's token-AA fix).
