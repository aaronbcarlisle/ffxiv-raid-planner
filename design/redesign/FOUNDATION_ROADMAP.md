...
> **Progress:** **F0 ✅ · F1 ✅ · F2 ✅ · F3 ✅ · F4 ✅ · F5 ✅ — all merged into `redesign/foundation`** (F0 #157, F1 #155, F2 #158 `2ffde63`, F3 #159 `ee03eab`, F4 #160 `2e26e02`, F5 #162 `5195d45`). **F6 is now under way as shell-first slices (§2.1): F6a ✅ merged (#163, squash `c95db5d`); F6b ✅ done (PR pending squash-merge) — F6c (Roster) is NEXT.** Each phase = a branch off `redesign/foundation` → PR → squash-merge into foundation; `foundation → main` lands everything at the end. Release notes are at **v2.0.2** (F4/F5/F6a were doc-only/internal, no bump).
...
| **F6b — Home** | Weekly-loop dashboard (`TwoRegionDashboard`); SessionRsvpCard (next-session), WeeklyLootSummaryCard, RosterReadinessCard, AttentionRow, StaticActivityFeed, TrackCard. Wired by passing `<Home/>` as the `overview` slot to `GroupViewContent` (slots architecture from F6a); legacy `StaticHomeTab` stays the fallback. Resolve the v2 settings-host mount here (pre-exposure). | F6a | ✅ **DONE** — PR pending squash-merge into `redesign/foundation`; settings-host mounted in `NewShell` (pre-exposure must-do resolved) |
