...
> **Status:** In execution · updated 2026-06-30 · integration base `redesign/foundation` (head `a25a971`)
> **Progress:** **F0 ✅ · F1 ✅ · F2 ✅ · F3 ✅ · F4 ✅ · F5 ✅ — all merged into `redesign/foundation`** (F0 #157, F1 #155, F2 #158 `2ffde63`, F3 #159 `ee03eab`, F4 #160 `2e26e02`, F5 #162 `5195d45`). **F6 is under way as shell-first slices (§2.1): F6a ✅ merged (#163, squash `c95db5d`); F6b ✅ merged (#164, squash `a25a971`) — F6c (Roster) is NEXT.** Each phase = a branch off `redesign/foundation` → PR → squash-merge into foundation; `foundation → main` lands everything at the end. Release notes are at **v2.0.2** (F4/F5/F6a/F6b were doc-only/internal, no bump).
...
| **F6b — Home** | Weekly-loop dashboard (`TwoRegionDashboard`); SessionRsvpCard (next-session), WeeklyLootSummaryCard, RosterReadinessCard, AttentionRow, StaticActivityFeed, TrackCard. Wired by passing `<Home/>` as the `overview` slot to `GroupViewContent` (slots architecture from F6a); legacy `StaticHomeTab` stays the fallback. Resolve the v2 settings-host mount here (pre-exposure). | F6a | ✅ **DONE** — #164, squash `a25a971`. 8 shared `ui/` components (CardShell, ProgressBar+Legend, PlayerIdentity, EmptyStateInvite, TwoRegionDashboard, AttentionRow, SessionRsvpCard) + 6 ring-0 `home/` components + Home wired via the `overview` slot; `home/`→ring0 classified (no new suppressions). **Settings-host mounted in `NewShell` — the F6a pre-exposure must-do is RESOLVED.** Browser-validated (v2 Home renders w/ real data; legacy byte-for-byte). Accepted F6b deviations (→F6c/polish): status-colored RSVP avatars (`ScheduleRsvp` has no role field), "Import BiS" attention rows shown to members. |
| **F6c — Roster** | Cards ⇄ Board (`SegmentedToggle`); CardShell, PlayerIdentity, ProgressBar(+Legend); GearBoard + GearBoardCell (re-homed gearsheet matrix / Split Planner). | F6a (Cards⇄Board axis shared w/ Loot) | **next** — reuses F6b's CardShell/ProgressBar(+Legend)/PlayerIdentity; SegmentedToggle is new |
