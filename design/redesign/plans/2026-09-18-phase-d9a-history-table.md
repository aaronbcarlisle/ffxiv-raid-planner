...
| D9a-j | **Header anatomy.** A `COLUMNS` const (`field`, `label`, `thClassName`, `align`) drives seven `ui/SortableHeader`s; the eighth `<th scope="col">` holds `<span className="sr-only">Actions</span>` (see the eslint constraint for the precedent reconciliation). `<caption className="sr-only">Loot and material history for this tier</caption>`. `<thead className="sticky top-0 z-10 bg-surface-card border-b border-border-default">`; the card wrapper is `rounded-lg border border-border-default bg-surface-card overflow-clip`. Widths: Week `w-16`, Floor `w-24`, Method `w-24`, Type `w-36`, ⋮ `w-12`; Slot/Player/Date auto. (Type was planned `w-28`; the final review wave raised it to the shipped `w-36` + `whitespace-nowrap` so `aug tome wpn` fits on one line — R-29 build note.) | Director vets ✓ |
...
| D9a-n | **`overflow-clip` means no horizontal scroll** (director F-1). `overflow-x-auto` (the `LogWeekGrid.tsx:612` idiom) would create a scrollport and defeat the sticky header, so R-D9a-D forces this trade: the card cannot scroll sideways, so below the width where eight columns fit it *would* clip its right edge. **Superseded by measurement (2026-09-18, after Type went `w-36`):** that width is never reached at any size tested — min-content 835 px at 1440, a filled 874 px card at 1024, min-content in an 837 px card at 900 with the ⋮ fully visible, and at 880 the *content pane* scrolls horizontally from the pre-existing stats-card row above the table while the card stays 837 px. The clip remains a property of `overflow-clip`, not an observed behaviour; the R-29 build note in `phase-d-loot-design.md` carries the numbers. Narrower is Phase P's, where the sticky/scroll trade is re-decided for mobile. | Director vets ✓ (new) |
...
- [ ] **Step 3: Implement** `LootHistoryTable.tsx`. Skeleton (the effect block `:69-103` is kept
  ...
  const COLUMNS: ReadonlyArray<{ field: HistorySortField; label: string; thClassName?: string; align?: 'left'|'center' }> = [
    ...
    { field: 'date', label: 'Date' }, { field: 'type', label: 'Type', thClassName: 'w-36' },
  ];
