...
- **U-8 (#5):** the roster card header is one line — tank role + position chips + job swap sit inline after the name; the job badge moves outside the avatar so both initials show. **SUPERSEDED — see "Outcome (2026-09-25)" below (option 2b).**
...
- **U-8/R-E2-D amended to option 2b** during Task 2: the header row = avatar + name + ONE seat chip
  + iLvl + kebab; SUB/`+N` ride the subtitle; the job swap moves into the kebab's "Change Job"; a
  tank's role + position merge into the new V2-only `TankSeatSelector`, one popover, that stays open
  after a pick (rather than closing per pick like the two selectors it replaces). The original
  U-8/R-E2-D lines above are marked superseded in place, not rewritten.
- **PR split:** the 2b ruling tripled Task 2's size (avatar-badge relocation + subtitle rework +
  `TankSeatSelector` + the shared `playerBisProgress` helper), pushing the slice's combined
  code+test total to 2,328 lines — over the ~1,500-line PR budget. Ships as two stacked PRs: **E2a**
  (plan + Task 1 + Task 3 + Task 4) and **E2b** (Task 2), reviewed and merged in that order.
- **Parked, with homes:** #3 → the Player Hub (spec approved 2026-09-25, PH1 next); #8, #32 → Phase
  F; #20, #22 → their own slices; #34 → deferred; the V1-shared skipped-heading h3s
  (`AvailabilityGrid.tsx:531`, `TemplateRecommendations.tsx:48`, `LootPriorityPanel.tsx:509`) →
  whenever V1 is authorized; D1 (a partial adjustments failure also opens the shell error overlay) →
  the holistic review; D3 (the delete-confirm modal's initial focus lands on `Checkbox`'s `sr-only`
  input) → the holistic a11y pass; D4 (Board's per-player `playerBisProgress` column vs. the
  `bisSlotTotals` aggregates elsewhere) → Phase F with #8; lint's "warnings ≤ N" gate not being
  CI-enforced → repo setup. Full detail: `ROLLOUT_ROADMAP.md` §7.
