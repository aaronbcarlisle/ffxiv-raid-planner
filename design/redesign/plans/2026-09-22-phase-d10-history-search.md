...
| **R-D10-Q** | An **unterminated** quote groups to end-of-string (R-15) but does **not** buy exact-match semantics — the value is treated as bare | Ruled at Task 1 build, where the implementer flagged it as unspecified and chose exactness. `player:"Tank` is a query *mid-typing*; exact-matching it empties the table for precisely the reason the neutral trailing colon exists (R-30). Leniency is the consistent answer |
...
| `week` | `parseInt(value, 10)` equals `weekNumber`; a non-numeric value matches nothing **and is reported in `unknownValues`** (R-D10-K) |
