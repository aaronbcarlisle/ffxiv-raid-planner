...
- New `lib/chromeContext.ts` (plain .ts — sidesteps react-refresh/only-export-components). NewShell wraps its **current** tree in the provider (temporary, removed in T3 when AppChrome takes over) so the escape item never disappears from group v2 mid-stack.
