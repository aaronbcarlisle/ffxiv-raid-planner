...
- **Done & merged into `redesign/foundation` (head `c95db5d`):** **F0** (locked decisions, #157) · **F1** (token pipeline, #155) · **F2** (anti-regression scaffolding, #158 `2ffde63`) · **F3** (component contracts + illegal-states, #159 `ee03eab`) · **F4** (frontend structure, #160 `2e26e02`) · **F5** (mockup validation pass, #162 `5195d45`) · **F6a** (the in-static shell behind `?shell=v2`, #163 `c95db5d`) · **F6b** (redesigned Home + settings-host mount, PR pending squash). Release notes at **v2.0.2** (F4/F5/F6a/F6b were internal/doc-only). **Pre-exposure must-do resolved:** the v2 static-settings panel is now mounted in `NewShell` via `StaticSettingsHost` (F6b Task 1) — `SettingsGear` and palette "Open Settings" are fully wired.
- **Next:** **F6c — Roster** (Cards ⇄ Board). F6 runs as shell-first vertical slices F6a–F6e (`FOUNDATION_ROADMAP.md §2.1`); F6b shipped the redesigned Home wired via the `overview` slot (`GroupViewContent`), plus 8 new shared `ui/` components (CardShell, ProgressBar, ProgressBarLegend, PlayerIdentity, EmptyStateInvite, TwoRegionDashboard, AttentionRow, SessionRsvpCard) and 6 ring-0 `home/` components. *(F5 shipped the build manifest: `specs/f5-screen-components-map.md` — 7 element-set tables tagging every Ring-0 element existing/refine/new against real component paths, a 27-entry new-component catalog, and the predecessors→one-owned-component consolidation map. F4 shipped the feature-slice + shared-layer model: `FRONTEND_STRUCTURE.md`, the Ring-aware `eslint-plugin-boundaries` graph + 28-edge suppressions baseline, and the backend Person↔Static note.)*
...
**F0–F5 are done and merged**, and **F6 is under way as shell-first slices (§2.1): F6a (the shell) is merged (#163 `c95db5d`).** F6 runs Rail → app shell (top bar + spine) → Home → Roster (Cards⇄Board) → Loot (Priority + unified recipient picker + History) → Schedule, each screen **consolidating its duplicated predecessors into one owned, contracted component** with its contract written alongside. Remaining:

- **F6b — Home (DONE, PR pending squash).** Built the weekly-loop dashboard from the F5 catalog (`TwoRegionDashboard`, `SessionRsvpCard`, `WeeklyLootSummaryCard`, `RosterReadinessCard`, `AttentionRow`, `StaticActivityFeed`, `TrackCard`, `CardShell`, `ProgressBar`+legend, `EmptyStateInvite`) and wired it via the `overview` slot on `GroupViewContent`. **Settings-host mount resolved:** `StaticSettingsHost` now mounts in `NewShell` — the pre-exposure must-do is closed.
- **F6c — Roster (NEXT).** Cards ⇄ Board (`SegmentedToggle`); `PlayerIdentity` board-cell variant; `GearBoard` + `GearBoardCell` (re-homed gearsheet matrix / Split Planner). Then F6d Loot → F6e Schedule.
- **As each slice rebuilds a Ring-0 domain:** prune its `frontend/eslint-suppressions.json` entries (`--prune-suppressions`) and re-enable the contrast harness on that screen. When all five slices merge and `/v2` is at parity, flip it to default and delete the legacy `GroupView` chrome. The `/docs/design-system` page rebuild is **post-F6**.
- **F6's build manifest is `specs/f5-screen-components-map.md`** — the 27-entry catalog (each unbuilt component's purpose + precursors + consolidation mandate) and every `refine` row's specific change to a real component.
