...
- **Kebab (audited):** BiS & Gear (Import/Update/Unlink BiS · BiS Targets · Weapon Priorities · Reset Gear) · Player Management (Flex Roles · Mark Sub/Main · Take/Release/Assign ownership) · Clipboard (Copy · Copy URL · Paste · Duplicate) · Danger Zone (Remove). **Re-homed out:** Edit Books · Adjust Priority → Loot (F6d); Lodestone Sync → the character link. ⚠ *SUPERSEDED IN PART (C7, 2026-07-27): "Edit Books" returns to the v2 kebab as a **navigation item only** — matrix D-05 (ruled 2026-07-26) restores the jump, and legacy's item was itself only a jump (`PlayerCard.tsx:388-398` → `handleNavigateToBooksPanel`). The books EDITING home stays re-homed to `BookLedgerCard`, which is what this audit decided.* ⚠ *ALSO SUPERSEDED IN PART (C8, 2026-07-28): "Lodestone Sync → the character link" landed as **the Characters modal**, not a card/kebab link — matrix D-12 (ruled 2026-07-26) and the flow map §3 both name the Characters path as the home, and the C8 ruling (2026-07-27) put the entry in the v2-owned `CharacterManageBridge`. What this audit got right stands: no Lodestone item returns to the v2 kebab.*
All permission-gated by the same fns; no trailing-arrow glyphs (§4.1).
...
| Player-card kebab (audited) | roster/gear/clipboard items stay on `RosterCard` via `useRosterCardActions` (reused modal components); **Edit Books · Adjust Priority → Loot (F6d)** *(Edit Books returned as navigation-only in C7 — see §5.5)*; **Lodestone Sync → the per-static character link** *(shipped in C8 as the Characters modal instead — see §5.5)* |
