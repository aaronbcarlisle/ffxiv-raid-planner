...
| **D-12** | **Lodestone character sync** — kebab "Lodestone Sync / Re-sync" and the whole search→preview→sync→compare modal (search, mock mode, URL/ID paste, linked panel, gear preview grid, sync, overwrite warning, identity-only fallback, post-sync compare, mismatch notices). *Rider: the card's sync **status line** (R-072) survives in reduced form as V2R-17 and is accounted in §1.K — restoring the flow should restore its character/server and job-mismatch detail too.* | **LOST** *(was CHANGED — re-classified by the fact-check)* | R-041, R-132, R-133, R-134, R-135, R-136, R-137, R-138, R-139, R-140, R-141 | **✅ VERIFIED (fact-check Q3): `RosterCharacterPanel` has NO Lodestone search** — only "Link Player Hub character" (`RosterCharacterMemberCard.tsx:85-91`), "Add manual character" (`:92-101`) and a passive `CharacterSyncBadge`. The same panel is mounted by **both** shells (legacy `GroupViewContent.tsx:948`, v2 `CharacterManageBridge.tsx:31`), so V2R-06 is not a re-home. With the card kebab gone, **`LodestoneSearchModal` has no entry point anywhere in v2's in-static surfaces** | no data | **REDESIGN** *(re-home the flow into the Characters modal / Player Hub path rather than the card kebab; rider R-072 detail restores with it; ruled 2026-07-26)* ⚠ *Rider amended at the C1 checkpoint (2026-07-26): the R-072 sync-line detail returns as a REDESIGNED, leaner treatment — v1's sync block bloats the card ("too much information… we can do better in v2"); the character/server + job-mismatch information is kept, the form is new (C5).* ✅ *Rider SHIPPED in C5 (2026-07-26): the footer sync line names the character (+ sync age); server, sync provenance and the job-mismatch explanation live in the hover detail; a mismatch also keeps a visible warning glyph + warning-colored dot, so it never hides behind the hover (live in `c5-sync-mismatch-dark.png`). No sub-12px text (legacy's block ran at 11px). Director-fold refinements (2026-07-26): the line renders as TWO segments — the name truncates at crowded widths, the age never does — and the detail hover is a `LongPressTooltip` (same pattern as the iLvl panel, keeps a touch path); keyboard/AT reachability of the hover detail rides the consolidated end-phase mobile/a11y pass. PR-#193-review fix (2026-07-26, Copilot): sync STATUS keys on `lastSync` itself, not Lodestone identity — a Player Hub claim auto-links sync data with no lodestone fields (`tiers.py _auto_link_bis_from_hub`), and the line now reads "Player Hub · synced Xh ago" there instead of contradicting the equipped-first headline with "Not synced"; the job-mismatch warning fires on hub syncs too. (Legacy had the same blind spot invisibly — its sync block simply didn't render without Lodestone identity; v1 byte-frozen, debt recorded here.) The flow re-home itself remains C8.* ✅ **FLOW RESTORED (C8, 2026-07-28)** — the entry lands in the **v2-owned** `CharacterManageBridge.tsx` (a "Lodestone sync" section inside the Characters modal, one row per **configured** player **including substitutes**), NOT in `RosterCharacterPanel`, so the §2.2 shared-surface exception is resolved by structure: zero V1-visible change, no shell gate, no shared-file hunk. **R-132…R-141 were never rebuilt** — they already shipped inside `LodestoneSearchModal` and were LOST only because nothing in v2 opened it; the deliverable was the entry point (R-041). **Two user rulings opened the slice (2026-07-27, both the plan's §5 silent defaults):** C8 stays in Phase C (not slid to Stage 3), and the mount is the v2-owned wrapper (the third option — an explicit V1-visible delta — was rejected: legacy already reaches the flow from its kebab, so a shared-panel entry would add a redundant V1 entry that fixes nothing). **Recorded deltas vs legacy:** (1) the entry is a per-player row in a Characters modal rather than a card kebab item, and it names the linked character + world where legacy's kebab named nothing; (2) opening the sync flow **closes** the Characters modal and closing it returns there — legacy never stacked these, and stacking is genuinely unsafe here: each `Modal` registers its Escape handler on `window` and the **first registered wins** the `stopImmediatePropagation` (`Modal.tsx:61-66`) — with both open that is Characters, so Escape would shut the modal *behind* the user while the topmost sync modal never saw the key at all, and the two Tab focus traps would compete on `window` for the same reason *(director-corrected 2026-07-28 — the first draft of this row and the code comment both claimed Escape would close **both**, which `Modal.tsx` refutes; the hand-off behaviour stands, the reason was wrong)*; (3) legacy's copy is preserved verbatim (`Lodestone Sync` / `Re-sync Lodestone`, `PlayerCard.tsx:358`), extended for AT with the player name; (4) a denied row states its reason **on screen** rather than in a `title`, because a disabled `Button` is `pointer-events-none` and a native tooltip can never fire (legacy's `ContextMenu` item is only `cursor-not-allowed`, so its `title` does show). **Permission parity held deliberately:** the row gates on per-player `canEditPlayer` (legacy's R-041 gate) with **`isAdminAccess`** as the admin argument — not the bridge's roster-level `canEdit`, and not the raw `isAdmin` flag, so View As downgrades the row exactly as it downgrades a card (`PlayerCard.tsx:319-320`). A member can sync their own claimed card, live-proven. `tierId` is threaded through because the modal's own post-sync `fetchTier` (`LodestoneSearchModal.tsx:300-302`) is what refreshes the roster; without it the sync would land silently.* |
...
| D-ID | What | Class | Legacy rows | V2 today | Usage signal | Ruling |
|---|---|---|---|---|---|---|
| **D-22** | **The "Who Needs It" matrix** — legacy's *default* priority view: player × slot dot grid (click a dot to log), FREE/count column, material pie-donut cells, mobile chip cards with ×N counters, the legend, and the all/F1-F4 floor tabs that dim it. | LOST | L-30, L-37, L-38, L-39, L-40, L-41, L-42, L-43, P-03 (loot-perm) | **absent.** V2L-01 states it plainly: v2's Priority view is per-floor `FloorCard` ranked queues (V2L-08); `WhoNeedsItMatrix` is imported only by the legacy panel and does not appear in the v2 tree | loot tab 514/76; **sub-tab split unmeasurable** | **RESTORE** *(matrix returns as a v2 Priority view alongside the floor-card queues — see D-23's switcher; ruled 2026-07-26)* → **✅ BUILT (slice D3, 2026-07-30/31)** as `NeedMatrix.tsx` / `needMatrixData.ts` — floor-banded F4→F1 rows, gear cells routed through the picker (R-4), Need column reads the rendered roster (R-11). Three named deltas from legacy, all deliberate: **the material pie-donut treatment is RESTORED**, re-expressed v2-owned (user-ruled 2026-07-31, reversing the interim D3 count-dot) — `MaterialProgressRing` re-expresses `MaterialPieIndicator`'s segmented-ring idea through a different mechanism (CSS conic-gradient, no SVG / stroke-dash — jscpd gate), still pool-faithful (count = the remaining need, not a raw total); **mobile chip cards are deferred** to the consolidated end-phase mobile pass (Phase P — the standing mobile ruling, not a regression); and **v2 FILTERS out-of-scope rows** instead of dimming them in place (ruled by R-48 — the D1 pill row is the scope control, so a filtered-out row has nothing left to dim against) |
| **D-23** | **The Loot Priority sub-tab axis** — Who Needs It / Gear Priority / Weapon Priority as three peer views, with mobile swipe between them, responsive header title, and remembered sub-tab. | CHANGED | L-15, L-16, L-17, L-18, L-21, L-22 | V2L-01 replaces the axis with a single Priority view (four floor cards) ⇄ History; weapon priority becomes a collapsible bridge (→ D-27) | unmeasurable | **REDESIGN** *(no legacy sub-tab axis; Priority view gains an internal switcher — Queues ⇄ Matrix — to host the restored D-22; ruled 2026-07-26; **widened to Queues ⇄ Matrix ⇄ Weapons by R-3**)* — **D1 (2026-07-30) shipped the Queues ⇄ Weapons half with per-user persistence; the Matrix segment and the R-1 landing land in D3, which completes this row** → **✅ BUILT — row closes (slice D3, 2026-07-30/31):** `Loot.tsx` adds the `matrix` `PriorityView` segment to the persisted switcher (`v2-loot-priority-view`); unset/unknown state lands on Matrix per R-1, mounting `NeedMatrix` scoped to `all` (the whole-tier read is Matrix's purpose) while Queues/Weapons keep their own floor defaults |
| **D-24** | **Floor selection** — F1–F4 filter pills that scope the priority list to one floor (shared `FilterBar`). | CHANGED | L-24, L-35 | ~~V2L-08 renders **all four floors at once** as cards~~ **⚠ superseded by the D1 build (2026-07-30):** the `All + F1–F4` pill row scopes the whole Priority view (R-2), the landing default is the newest in-progress floor latched per tier visit (R-10), and auto-collapse is conditional — off for a solo-scoped card (R-49, `FloorCard.tsx` `autoCollapse`) | `loot_log` by fight: M9S 24 · M11S 19 · M10S 17 · M12S 6 | **REDESIGN** *(floor scoping rethought in Phase D — cards stay, scoping mechanism to be designed; ruled 2026-07-26)* → **✅ BUILT as the R-2/R-10 pill row (slice D1, 2026-07-30)** |
| **D-25** | **Priority-score transparency** — the hover breakdown on every score badge (role / need / job / player / loot-adjust / drought / balance; weapon: main-job bonus / role / rank) and the "Loot history adjustments active" badge. | LOST | L-20, L-27, L-59 | absent from the v2 inventory — `PriorityRow` ranks recipients but no score-breakdown affordance is listed | no data | **RESTORE** *(score-breakdown hover + adjustments-active badge on v2 ranked rows; ruled 2026-07-26)* → **✅ BUILT — both halves (slices D2/D3, 2026-07-30/31).** The breakdown-hover half: `ScoreBreakdown` (role / need / job / player / loot-adjust / drought / balance, one line per nonzero part) headlines with the **enhanced-final score** — `entry.enhancedScore ?? entry.score`, the exact sort key, matching legacy `LootPriorityPanel.tsx:53`'s `displayScore` parity — inside `RecipientPicker`'s ranked rows (D2) and the queue rows' `QueueWhy` popover (D3). The adjustments-active half: the "Loot history adjustments active" line surfaces in both the picker and `QueueWhy`, gated on enhanced scoring being on AND a non-empty loot log (`enhancedActive`); a per-row **Adjusted** `Tag` additionally fires wherever `lootAdjustmentBonus` or `playerModifier` is nonzero (`hasAdjustments`), so an individual row's history-driven nudge is visible even when the surface-level line doesn't apply |
| **D-26** | **Logging from the priority list** — per-slot "Log" buttons, per-material "Log" buttons, and "+ Log Floor" (single-floor wizard). | CHANGED | L-25, L-26, L-28, L-34, P-01, P-02 (loot-perm) | V2L-09 per-row **"Assign"** → `RecipientPicker` fixed to that slot/material; V2L-12 opens the wizard **week-scoped only**. ~~**No single-floor wizard entry exists in v2.**~~ **⚠ superseded by the D1 build (2026-07-30): R-7's `Log floor — {duty}` runs `LogWeekWizard` in `singleFloorMode`, following the pill's floor (Weapons pins floor 4; on All it steps aside for the whole-week button).** ⚠ **Coupled to D-30:** legacy's *other* single-floor entry is the weekly grid's floor-header "Log Floor" (L-142, inside D-30). Ruling **D-30 `drop` + D-26 `restore`** would leave single-floor logging with no home at all — decide the pair together | `loot_log` 66/17 · `material_log` 17/5 | **RESTORE** *(keep v2 per-row Assign; "+ Log Floor" single-floor wizard entry returns on each floor card — resolves the D-30 coupling; ruled 2026-07-26)* → **✅ BUILT — Priority's half landed as ONE view-level button following the pill (R-7, slice D1, 2026-07-30; R-7 deliberately supersedes "on each floor card"); Log's half (R-25 floor-kebab entry) lands with D5/D6** |
| **D-27** | **Weapon Priority as a top-level view** — its own sub-tab with the full role-sectioned list in the main content area. | CHANGED | L-29 | ~~V2L-13 demotes it to a collapsible "Weapon priorities" link in the **Floor-4 card footer**~~ **⚠ superseded by the D1 build (2026-07-30):** Weapons is the third segment of the Priority switcher (R-3), its body the same legacy `WeaponPriorityList` mounted verbatim inside a floor-4-identified card (`WeaponPriorityBridge.tsx`); the footer disclosure is gone | unmeasurable | **REDESIGN** *(promote to a more visible entry than the Floor-4 footer link — own card or peer entry, designed in Phase D; ruled 2026-07-26)* → **✅ BUILT as the peer switcher segment (R-3, slice D1, 2026-07-30)** |
| **D-28** | **QuickLogDropModal** — week `NumberInput`, priority-labelled recipient select ("Top Priority"/"2nd"/"3rd"), "Mark X as acquired", weapon-only "Extra loot", the "This will:" action preview, and its footer. | CHANGED | L-31, L-61, L-63, L-64, L-65, L-66, L-67 | V2L-11 `RecipientPicker` (mode=assign/log/edit): scope toggle (By priority / All members / Off-spec-free), search, ranked radio rows, method (drop/tome/purchase/**book**), week, extra/free checkbox, notes, character auto-pick. ~~**Superset in scope; no "this will:" preview, no explicit acquired checkbox evidenced**~~ **⚠ superseded by the D2 build (2026-07-30):** consequences are now stated without a disclosure click — a live "This will:" preview mirrors the real write-path predicates, and the acquired checkbox sits in the modal body (disabled, captioned "Gear sync applies to drops and books." under tome/purchase); the expandable "Details" section is now "Options" (R-12) | no data (`modal_open` dead — caveat 3) | **KEEP V2 + ADDITIONS** *(RecipientPicker stands; add the "This will:" action preview + acquired visibility; rename the expandable "Details" section to "Options"; ruled 2026-07-26)* → **✅ BUILT (slice D2, 2026-07-30)** |
| **D-29** | **Loot recommendation candidates** — the ranked candidate panel inside the log modals: click-to-fill, Main/Alt/Player badges, per-candidate reasons + warnings tooltip, "already received" chip, show-more/fewer, and a high/medium/low **confidence** header. | LOST | L-62, L-68, L-69, L-70, L-71, L-72, L-178 | absent — V2L-11 has a ranked list and character auto-pick, but no reasons/warnings/confidence layer | no data | **RESTORE** *(full reasons/warnings/confidence candidate layer inside RecipientPicker; ruled 2026-07-26)* → **✅ BUILT — row closes (slice D3, 2026-07-30/31).** R-6 narrowed the restore to reasons + warnings + confidence (Main/Alt/Player badges, the "already received" chip, and show-more/fewer are consciously NOT restored — matrix cells and queue rows don't carry the log modals' alt-character bookkeeping). The picker's half (ranked rows, per-candidate reason line, high/medium/low confidence header) shipped D2; D3 completes the row by threading the same `RankingExplanation` leaf, warnings included, into the queue rows' `QueueWhy` popover — a queue row and its equivalent picker row can never disagree about why a ranking is what it is (R-6's invariant); matrix cells stay minimal (plain "Log X for Y" tooltips, no explanation layer) |
...
| **D-31** | **All Weeks view — the cross-week TABLE** *(split: search moved to D-72)*: a whole-tier flat table with All/Gear/Materials toggle, floor chips, **sortable columns** (Week/Floor/Slot/Player/Method/Date/Type), row click-to-edit, a row context menu including "View Week N in Grid/List", stats **header** (⚠ *this row said "footer" until 2026-07-28; it renders above the table, `AllWeeksView.tsx:505-511`*) and filtered-vs-empty states. | LOST | L-107, L-158, L-159, L-160, L-161, L-162, L-163, L-164, L-165, L-166 | **absent** — V2L-16 groups by week; there is no flat cross-week table and no column sorting anywhere in v2 | era-1 history 366/70; layout split unmeasurable | **RESTORE — AS THE HISTORY MODEL** *(user ruling: this IS what the History tab should be — filter/sort/find previously logged entries; lean v1's table over v2's week-grouped list, merging v2's pills. Part of the D-30 split; ruled 2026-07-26)* |
...
| **D-35** | **Free-form log entry points** — "+ Log Loot" and "+ Log Material" buttons opening a blank modal (and their Alt+L / **Alt+U** shortcuts — ⚠ *this row said Alt+M until 2026-07-28; corrected under R-20: legacy binds material to `Alt+U` (`useGroupViewKeyboardShortcuts.ts:185-191`) and **Alt+M is Settings: Members**, registered `alwaysEnabled` and therefore live in v2 today (`:213-217`). D-54 recorded it correctly all along.*), plus the toolbar "Log Week". | CHANGED | L-138, L-139, L-140 | V2L-10 "Log a drop" (free-form fight+slot) and V2L-12 "Log this week's loot" exist; **no free-form material entry point** — materials can only be logged from a floor row's Assign (V2L-09) | `material_log` 17 events/5 users | **RESTORE (MATERIAL ENTRY)** *(free-form "Log material" entry point + **Alt+U** returns (⚠ *cell said Alt+M until 2026-07-28; Alt+M is Settings: Members and is live in v2 — see the What column*); "Log a drop" already covers loot; ruled 2026-07-26)* |
| **D-36** | **AddLootEntryModal** (add + edit) — week/floor, floor-filtered slot select, recommendation panel, recipient select with **"Include Subs" / "Show all players"** widening checkboxes and the "No one needs this item!" hint, character selector, Drop/Book method, "also mark acquired", notes, and edit-mode recipient preservation. | CHANGED | L-175, L-176, L-177, L-179, L-180, L-181, L-182, L-183, L-184, L-185, L-186 | V2L-11 `RecipientPicker` mode=log/edit covers scope widening (By priority / All members / Off-spec-free), method (superset: drop/tome/purchase/book), week, notes, character auto-pick. **The recommendation panel does not survive (→ D-29)**; ~~the "no one needs this" hint has no stated v2 equivalent~~ **⚠ superseded by the D2 build (2026-07-30): the hint is present in the v2 picker**, restored, with the copy reading "No one needs this item for BiS — assigning it counts as a free roll." in create modes when the scoped slot has no needers | Phase-A item 12 already removed `disableAssign` so an empty queue is still assignable | **KEEP V2 + HINT** *(RecipientPicker stands; the "no one needs this" empty-queue hint returns; rec panel already restored via D-29; ruled 2026-07-26)* → **✅ BUILT — hint half (slice D2, 2026-07-30); rec panel's reasons/warnings/confidence landed narrowly via R-6 (Main/Alt/Player badges + show-more/fewer consciously not restored — R-6 narrowed D-29 to warnings + confidence; D-29's row closes in D3)** |
...
| **D-40** | **Week stepper** — sliding 3-dot week navigator with click-to-jump, per-week status dots (loot/books/mats logged) and their tooltips, prev/next chevrons, "Go to Current Week", "Start Next Week" and "Revert Week" inline, plus the mobile stepper row. | CHANGED | L-100, L-208, L-209, L-210, L-211, L-212, L-213 | V2L-02 week-scope **dropdown pill** (date range + loot/books/mats type dots) + V2L-03 Start-next / Revert. **No stepper, no "go to current week", no chevrons**; v2 also surfaces "Week N" in the TopBar (V2K-09, desktop-only) | no data | **KEEP V2 + CHEVRONS** *(dropdown pill stands; add prev/next stepping + go-to-current affordance; ruled 2026-07-26)* → **✅ BUILT (slice D4, 2026-07-31/08-07)** as `WeekScopeControl.tsx`'s prev/next `IconButton`s flanking the dropdown pill, plus a `LocateFixed` go-to-current `IconButton` (disabled, not hidden, once already current). **Scoped exactly as ruled:** the sliding 3-dot stepper itself is not restored — the dropdown pill stands. The mobile stepper row is out per the standing mobile-deferral ruling (Phase P). ⚠ **Known cold-load caveat, deferred to D5:** on a cold load of `?lview=log&week=N`, the store's provisional `currentWeek: 1, maxWeek: 1` (`lootTrackingStore.ts:115-116`) hasn't settled from `fetchCurrentWeek` yet, so `useLogWeek`'s bounds are briefly wrong — a Previous click inside that window can clamp to 1 and clear the override, and an out-of-range `week=` (e.g. 99 on a 4-week tier) is never re-clamped once the clock does settle. Needs a "clock has settled" flag on the store, outside D4's scope |
| **D-41** | **Revert-week safety** — the pre-check that fetches the latest week and, if anything is logged, shows a **data-summary modal listing the loot/materials/books that will move**. | CHANGED | L-102, L-104, L-206, L-207 | V2L-03 uses a plain confirm ("Move the clock back…"); **no data summary is evidenced** | no data | **RESTORE** *(pre-check + data-summary modal before reverting; ruled 2026-07-26)* → **✅ BUILT — in full (slice D4, 2026-07-31/08-07).** `WeekScopeControl.tsx`'s `handleRevertClick` fires the pre-check directly off the "Revert week" click: fetches loot/material/page-ledger fresh, then re-checks the clock's current week for data. Data found ⇒ `RevertWeekSummaryModal.tsx` — itemised loot/materials/book entries in a scroll container (no row cap, per the ruling), a counts-summary line, and (fix round 1) an optional `notice` line carrying the displayed-vs-clock divergence statement onto the mutating surface itself. Nothing found ⇒ a lightweight `ConfirmModal` instead, also carrying the divergence line. **One confirmation per path, never zero, never two** (director-ruled at task review, see the D4 plan's Task 3 write-back) |
