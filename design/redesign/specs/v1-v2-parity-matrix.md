...
|---|---|---|---|---|---|---|
| **D-01** | The **expanded ⇄ compact card axis** itself: two card densities, a mobile floating density FAB, re-click-to-expand-all, the `V` shortcut, and the expanded-only "active BiS target" chip. | LOST | R-014, R-015, R-023, R-065, R-066, R-087, R-165 | absent — v2 has one card density (V2R-10…V2R-17) | **expanded 203/71 vs compact 126/51** — strongest single datapoint in the matrix | **RESTORE** *(restyled — Phase C centerpiece; ruled 2026-07-26)* |
| **D-02** | **Editing and inspecting gear on the card**: click-to-cycle each slot (2-state raid/crafted/base-tome, 3-state augmented-tome), hover item card with stats/materia/equipped-vs-BiS, the Status-column tooltip that teaches the cycle. | LOST | R-088, R-093, R-096, R-097 | V2R-15 gear pips are **explicitly read-only** (`disabled`, no-op `onChange`); editing exists only on the Board (V2R-21/22) | no data — `player_gear_changed` never fired (caveat 3) | **RESTORE** *(click-to-cycle + hover inspect on the restored D-01 card; ruled 2026-07-26)* |
| **D-03** | **Per-slot BiS-source assignment**: the R/T/C/BT selector popover (with the reset-warning confirm), the per-slot "Fix" button, and the "N slots need BiS source updates" banner that bulk-corrects them. | LOST | R-086, R-091, R-092 | absent — no v2 affordance sets a slot's `bisSource`; `BiSSourceSelector` is unmounted in v2 | no data | **RESTORE** *(selector + Fix + bulk banner return with the restored gear table; ruled 2026-07-26)* |
| **D-04** | **Tome-weapon tracking**: the weapon row's "+" toggle and the tome-weapon sub-row with its own 3-state circle and material-entry jump. | CHANGED | R-094, R-095 | V2R-14 kebab item "Track / Stop Tracking Tome Weapon" — state toggles, but the sub-row (and its own progress circle) is gone | no data | **RESTORE** *(sub-row + own progress circle + material jump; ruled 2026-07-26)* |
...
| **D-18** | **Split Clear Planner** — the whole surface, reachable from the roster sub-tabs and from the More page. | LOST | R-186, M-4 | **not rendered in v2** — v2 never wires `onOpenSplitPlanner` (V2M-04); explicitly the "clearest v2-dropped row" in the More-page audit | no data | **RESTORE** *(wire the existing surface into v2; entry point from More per D-17's Keep-V2 ruling; ruled 2026-07-26)* |
...
| **D-20** | **Static/tier error modal** — API error with a technical-details block, "Copy" (2s confirm) and "Report Bug" → Discord. | LOST | R-005 | absent from the v2 inventory (lives in the legacy `GroupView.tsx`, which v2 does not mount) | `error_reports`: **517 rows / 78 users** (255 unhandled_rejection · 254 backend_error · 8 js_error) | **RESTORE** *(error modal with details + Copy + Report Bug returns under v2; ruled 2026-07-26)* |
