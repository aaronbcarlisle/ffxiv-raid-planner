...
| **D-20** | **Static/tier error modal** — API error with a technical-details block, "Copy" (2s confirm) and "Report Bug" → Discord. | LOST | R-005 | absent from the v2 inventory (lives in the legacy `GroupView.tsx`, which v2 does not mount) | `error_reports`: **517 rows / 78 users** (255 unhandled_rejection · 254 backend_error · 8 js_error) | **RESTORE → ⚠ CORRECTED: ALREADY SHIPPED** *(ruled 2026-07-26; Phase-C plan vet same day found the modal **already exists in v2** — `ShellContentStates.tsx:215-273`: details block, 2-s Copy confirm, Report Bug → Discord, v2-only mount `NewShell.tsx:128`. The "absent" claim came from the v2 inventory doc, not code. Treated as KEPT; Phase-C closeout verifies incl. no double-modal)* |
