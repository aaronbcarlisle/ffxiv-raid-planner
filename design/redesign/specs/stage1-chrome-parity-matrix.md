...
**Post-sign-off edits (2026-07-25, PR-3 review — content unchanged unless noted):** (1) line cites re-anchored to the implemented branch per plan §3 T6/director G (two passes: `e9f849c` + the RC-C completion); (2) H15's baseline claim now carries a "(pre-Stage-1)" qualifier — the signed wording described the pre-flip world and briefly read as a post-flip claim; (3) recorded delta on §1's mobile-bar "theme stays in the user menu" sentence: for **guests** (who have no user menu) the implemented mobile bar shows Discord/GitHub + ThemeToggle inline — a superset of legacy mobile (which offers guests no theme control), adjudicated APPROVED at PR-3 change-review.
...
Group-only affordances (tier selector/kebab `:250`, Invite `:274`, TipsCarousel `:313`, TryNewUiBanner desktop `:325` + mobile `:444`, mobile StaticSwitcher row `:421`) are all `isGroupRoute && currentGroup`-gated — verified N/A on non-group routes.
...
| H8 | Discord link (external, `Header.tsx:374`; **no `sm:` gate — renders on legacy mobile**) | always (non-`/`) | **KEPT (desktop) + OPEN (M2) (mobile)** | Desktop: slim TopBar right cluster. Mobile: recommend **RE-HOMED** to the v2 UserMenu (two `DropdownItem`s; the mobile top bar reaches it via the avatar) — a bare "accept desktop-only" would be an unjustified drop. |
| H9 | GitHub link (external, `Header.tsx:385`) | always (non-`/`) | **KEPT (desktop) + OPEN (M2) (mobile)** | Same as H8 |
| H10 | ThemeToggle (desktop `Header.tsx:396-399`; mobile lives in UserMenu) | always (non-`/`) | **KEPT** | Slim TopBar (desktop); UserMenu (mobile) — identical split to legacy |
| H11 | UserMenu (desktop `Header.tsx:409-415`; on `/profile` mobile-only via `hasOwnRailUserMenu` `:70`) | authed | **KEPT** | AppRail footer (desktop) / v2 mobile top bar avatar (<sm) |
