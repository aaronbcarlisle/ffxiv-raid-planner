...
These are shared *because* Home (ring0), Schedule (ring1), and Player Hub (person) all consume them (F5 whole-screen validation is the ≥2-consumer promotion evidence — not speculative) — and for `SessionRsvpCard`/`TwoRegionDashboard` shared placement is *mandatory* (a ring0 component and a ring1 component both import them; only the shared layer is importable by both). They must be fully token-clean (the shared layer locks DS color/type/interaction rules at **error**) and stay **presentational** (props-in / callbacks-out, no store imports — that's the shared-leaf rule). **Escalation guardrail:** if a "shared" component turns out during build to genuinely require store/feature coupling (can't stay presentational), that's a `NEEDS_CONTEXT` escalation — re-home it to the ring that owns the coupling, do not force it into `ui/` with a workaround.
...
**Promoted to `utils/` (shared by new Home + legacy tab) — promote-and-repoint, NOT copy:** `rosterAvgIlv`, `playerGearReadiness`/BiS-count helper, and `deriveActivityItems` → `utils/staticActivity.ts`. The new components import the `utils/` versions; the legacy `StaticHomeTab` is **repointed** to import them too (its local copies are deleted). **Why not copy-and-leave-legacy:** (1) the F2 `jscpd` gate is fail-on-new — a verbatim ~145-line copy of `deriveActivityItems` would be flagged as a new clone and break CI; (2) duplicating the *privacy* derivation invites the two copies to drift on exactly the rules (plugin-anonymization / visibility filter) that must not drift. Promote-and-repoint is a **pure-function move** (the F6a Task 2/3 extraction pattern, reviewed clean), made safe by: a **characterization test** on each extracted function (lock outputs across fixtures) + **verifying the legacy `StaticHomeTab` render is byte-for-byte** after the imports repoint + a **PR-body note** that the `StaticHomeTab` diff is import-path-only (behavior-neutral). This honors the "leave legacy alone" decision in substance (rendered output unchanged); the import repoint disappears entirely when legacy is deleted at the F6e cutover.
...
- **Activity-util promotion (decided: promote-and-repoint):** extracting `deriveActivityItems`/`rosterAvgIlv`/`playerGearReadiness` to `utils/` repoints a legacy import in `StaticHomeTab`. Copy-not-move is rejected (trips the `jscpd` fail-on-new gate + risks privacy-logic drift — §6). The plan must include a characterization test per extracted function + a byte-for-byte legacy-render check after repointing, and the PR body flags the `StaticHomeTab` diff as import-path-only for the reviewer/bots.
...
- **Ambiguity:** the "in progress" bar is defined as no-fabricated-percentage (cleared=full/success, else neutral + text); the Track card is unambiguously display-only; the CTA routes to Loot (not the wizard); the activity-util extraction is **decided** (promote-and-repoint, §6) with its safety net; the shared/ring0 split carries an explicit escalation guardrail (§4.1). No open calls remain for the plan beyond ordinary task-granularity choices.
