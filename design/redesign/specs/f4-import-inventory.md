...
| `mount-farms` | ring3 | |
| `collections` | ring3 | |
| `admin` | admin | **EXEMPT as importer** — outside ring graph; `admin/` may import across rings freely. However, `admin/` is a **disallowed import target** for product-ring code — `ring → admin` IS a violation (admin-ops must not leak into product rings). See §6. |
...
| `history` | ring0 | ⚠️ **violation** | 2 (AllWeeksView → admin/SortableHeader + admin/sortUtils; ring0→admin; grandfathered — see §6) |
...
- `history/AllWeeksView.tsx` imports `admin/SortableHeader` and `admin/sortUtils` (ring0 → admin). These **are** counted violations — admin is exempt as an **importer** (admin/ may reach across rings), but is a **disallowed import target** for product-ring code. These 2 edges are grandfathered in the ESLint baseline; see §6 for the correction and full reconciliation.
...
## 6. Reconciliation with the ESLint baseline (Task 4)

**The ESLint baseline is the source of truth.** The grep-based inventory above found **18** outward edges; the `frontend/eslint-suppressions.json` baseline generated by Task 4 found **28** genuine violations — all confirmed real (reviewer-confirmed, zero false positives). The lint supersedes this document for all accuracy purposes.

### 10-edge delta

| Delta | Source | Detail |
|-------|--------|--------|
| **+7** | `layout/` (shell → outer) | The grep did not scan `layout/`. Six layout files import from auth, settings, static-group, or admin: `GlobalSettingsPanel.tsx` (×1), `Header.tsx` (×2), `Layout.tsx` (×1), `SettingsDockToggle.tsx` (×1), `SettingsPanelController.tsx` (×1), `SidebarNav.tsx` (×1). Genuine shell→outer coupling; grandfathered. Lifts to page-level at F6 when the navigation rail is rebuilt. |
| **+2** | `history/AllWeeksView.tsx` (ring0 → admin) | This inventory initially excluded these as "admin-exempt" — that was wrong. See correction note below. |
| **+1** | `dashboard/MyStaticsPanel.tsx` (person → ring0) | Imports `wizard/` (Ring 0); missed by the grep scan. Confirmed by the ESLint baseline. |

### Correction: admin-as-target semantics

The earlier description of `admin` as "never counted as … to-target" (ring-tag map, now corrected above) and the initial exclusion of `AllWeeksView → admin/*` from the edge count were **wrong**.

The admin exemption applies only **as an importer**: `admin/` may reach across rings freely (it is a platform-ops surface, outside the ring graph). The inverse direction — `ring → admin` — **is a violation**: admin-ops code must not leak into product rings; coupling product screens to admin infrastructure is architectural debt. `history/AllWeeksView` → `admin/SortableHeader` + `admin/sortUtils` are therefore genuine grandfathered ring0→admin edges (count: 2), captured in `frontend/eslint-suppressions.json`.

### Authoritative baseline

`frontend/eslint-suppressions.json` (28 total violations, 17 files) is the live source of truth. It ratchets forward via `--prune-suppressions` as F6 rebuilds each domain. Per-file counts in that file supersede this grep-based document for all accuracy purposes.
