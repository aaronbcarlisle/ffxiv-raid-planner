...
- **Generator:** Style Dictionary v4 (first-class DTCG support), replacing the Python reference generators (`gen_tailwind.py`, `generate_css.py`) which proved the round-trip but aren't a real build step. Output is a build artifact; the JSON is the source.
...
- **CI:** a `pnpm` script regenerates and a CI check fails the build if committed generated CSS diverges from what the JSON produces (hand-edits to generated vars are rejected). Folds under the existing `check:design-system` gate.
