...
| **R-35 / R-42** | `ui/keyboardShortcutGroups.ts`, `ui/KeyboardShortcutsHelp.tsx`, `layout/CommandPalette.tsx`, `hooks/useGroupViewKeyboardShortcuts.ts` | **YES for the help modal, ~~and palette~~ — corrected 2026-09-22 (D11, R-D11-I).** The help modal renders in **both** shells; **`CommandPalette` does NOT** — it has exactly one production mount, `NewShell.tsx:4,330` (v2), every other reference in `src/` being a test, a `vi.mock`, a comment or a release note (verified by grep at D11 plan-vet). Editing the palette is therefore **not** a V1 reach. The dropped `Alt+1-3` / `V` / `G` rows are still live in V1 under `legacyLootSurface` (`GroupViewContent.tsx:510`) | The help modal must stay truthful for two shells → **shell-aware groups** (§5 open item). Editing the shared hook is a V1 reach in itself. **D11 took the minimal seam** (R-D11-C): an additive `V2_SHORTCUT_GROUPS` plus an optional `extraGroups` prop passed only by `Layout.tsx`'s v2 mount, so V1's help is unchanged. It is append-only, so it cannot express §5's default ("V1's rows stay, v2's reflect the new set") — **D14 decides whether to generalise or replace it** |
| **R-35** | `layout/Layout.tsx` | **YES, and on every route** — it renders the V1 shell (`:106-146`) and mounts `KeyboardShortcutsHelp` **twice**, once per shell (`:97-101` v2, `:140-144` legacy). **Added 2026-09-22 (D11 plan-vet, D-REC-2)**: the registry work necessarily touches it and the inventory omitted it | The v2 branch's mount only; the legacy branch stays byte-identical. ⚠ **This file is not a legacy-only path, so DoD 3 part (a) passes trivially for it** — the guard is a test that fails on the mutation (D11's T-29 in `Layout.chrome.test.tsx`), never the `git diff` |
...
| **R-D12-I** | `pages/GroupViewContent.tsx` | **YES, and on every route with a `?player=` deep link** — its `?player=` effect (`:242-270`) strips both `player` and `slot` on the same 2500 ms timer (`:260,265`), and the effect runs in both shells' mount tree. **Added 2026-09-23 (D12 plan-vet, D-REC-4)**: the one-line `params.delete('slot')` addition necessarily touches it and the inventory omitted it, exactly as it omitted `layout/Layout.tsx` until D11's plan-vet added it above as D-REC-2 | +5 (4 comment + 1 executable), `params.delete('slot')` (`:265`), inside the existing `?player=` strip. V1 never writes `?slot=`, so the delete is a no-op on every legacy path. ⚠ **This file is not a legacy-only path, so DoD 3 part (a) passes trivially for it** — the guard is the `GroupViewContent — D12 the ?player=/?slot= strip` describe block in `GroupViewContent.test.tsx` (`:204`, net-new coverage of this effect) and the battery row "GroupViewContent's 2500ms strip drops params.delete('slot')" in `scripts/d12-mutation-battery.py`, never the `git diff` |
| *(added 2026-09-25, D14b DoD 5)* | `loot/LogWeekWizard/**` — `index.tsx`, `BooksStep.tsx`, `ConfirmStep.tsx`, `GearStep.tsx`, `types.ts` | **YES** — V1's `LootPriorityPanel.tsx:30,788` and the barrel (`index.ts:5` ← `GroupViewContent.tsx:38`); v2 mounts it at `Loot.tsx:177,1555`. Shared since #67 (`b1b2da94`), long before Phase D | Never edited in Phase D — `git log 3922f1af..HEAD -- components/loot/LogWeekWizard/` is empty. DoD 2 pins its three store calls |
| *(added 2026-09-25, D14b director vet)* | The frozen `history/` modules v2 mounts: `EditBookBalanceModal`, `PlayerLedgerModal`, `MarkFloorClearedModal` (`BookLedgerCard.tsx:22-24`), `LootFairnessLegend` (`Loot.tsx:172`, from `WeeklyLootGrid.tsx`), `DeleteLootConfirmModal` (`Loot.tsx:189`) | **YES for the first four** — the three modals via `LootLogModals.tsx:17-19` ← `SectionedLogView.tsx:17,1794`, `LootFairnessLegend` via `SectionedLogView.tsx:23`. `DeleteLootConfirmModal`'s other importers (`LootLogPanel`, `UnifiedWeekOverview`) are orphaned, so v2 is its only live consumer | Imported unmodified (§2.2: *don't edit* `history/`); none of the five has a Phase-D commit. `PlayerLedgerModal`'s `deletePlayerLedger` is pinned by DoD 2 |

**Dead-code evidence (verified on `main` at `3922f1af`):** `loot/FloorSelector.tsx`'s only reference
is `loot/index.ts:2`. `loot/SummaryPanel.tsx`'s only reference is `loot/index.ts:7` (was cited `:8`) — **✅ closed
by D13 (#271): `SummaryPanel`, its barrel line and `team/TeamSummary.tsx` deleted (R-D13-I).** Original note — see §2.7:
...
**✅ Ruled and shipped by D13 (#271):** `TeamSummaryEnhanced` is the reference, re-expressed as the
v2-owned `home/TeamSummaryCard.tsx` in Home's main column (members only), proven by a render-level
parity test against V1; `SummaryPanel` and the plain `TeamSummary` are deleted.
...
| **D9a** | History table | Flat sortable table on D0's header; `createdAt` desc tiebreak (**not** v1's comparator — that would regress what ships); R-33 floor chip **on tokens**; R-38 `weaponJob`; R-39 slot icon; the eighth kebab column + R-34's `slotAugmented` readout (R-D9a-A) + sticky thead (R-D9a-D) | Pre-declared split (§4 sizing). `AllWeeksView.tsx` = reference → **✅ BUILT (PR #262)** |
| **D9b** | Separators + states | R-29's week separators **only under week sort**, carrying the current-week marker and its measured `text-accent-hover` token; R-34's stats count, filtered-vs-empty; the `?entry=`/`?entryType=` highlight already survived D9a on `historyRowDomId` — D9b's job is the test under separators. D9b's `rangeOfWeek` re-add also re-consumes `hooks/useWeekClock`'s `WeekRange`, closing D9a's +1 knip entry. — **✅ BUILT (D9b, PR #264)**, knip back to +0 (Unused exported types 141 → 140). | That highlight is **live v2 behaviour** and the destination of shipped C7 roster jumps (`RosterCard.tsx:281-297`) — it must keep resolving against the **unfiltered** logs (`Loot.tsx:39-43`), with a test |
| **D10** | History search | v2's **own** parser: R-47 aliases, R-36 comma alternation + `source:`, `player:"Tank One"` quoting, neutral trailing colon, surfaced unknown key; pills write tokens and light **only on exact equality**; R-37 session-local — the query never enters the URL, so `copyLink` has **nothing to strip** and the *absence* of a `delete('q')` is load-bearing (R-D10-F). — **✅ BUILT (D10, PR #265)**: `HistoryFilters.tsx` + `DEFAULT_HISTORY_FILTERS`/`HistoryFilterState`/`HistorySource`/`filterHistoryItems`/`historyWeeks` all deleted, zero code references left; knip +0. | `HistoryFilters` / `DEFAULT_HISTORY_FILTERS` dissolve into the query string |
| **D11** | History row affordances | R-31 (click edits; cursor **only** when `canEdit`; text selectable; `Alt+Enter`/`Shift+Enter`; the `tabIndex` `<tr>` gets a `role`); R-32 (kebab incl. **View week N in Log**; material Edit → D8's modal; `jumpMenuAnchor` from `rosterLedgerJumps.ts:75-83`, not raw `clientX/clientY`); R-35 `Ctrl+Shift+F` with a focus/modal guard **and** a registry entry. — **✅ BUILT (D11)**: rows are controls only where they can act (Shift/Alt live for viewers, `tabIndex`/`role`/`aria-label` editor-only, no `select-none`); the kebab is now the two-trigger `ui/ContextMenu` carrying **Edit · Copy link · Jump · View week N in Log · Delete**; material edit reaches D8's modal, closing R-34's last ⚠half row; `Ctrl+Shift+F` registers v2-locally through the shared hook with three gates. R-D11-A…N in the slice plan | The registry is shared (§2.1) — the `Shift+?` entry must not change V1's help. **Held**: `SHORTCUT_GROUPS` is byte-identical and a v2-only `extraGroups` prop is passed by `Layout.tsx`'s v2 mount alone, proven by a test that fails on the legacy-mount mutation (part (a) cannot — see the `Layout.tsx` row above) |
| **D12** | The jumps | R-18's destination: `RosterGearTable` gains `gear-row-{playerId}-{slot}` anchors + `highlightedSlot` pulse (net-new — legacy's live only in frozen `GearTable.tsx:324,659`); R-28's split (displayed week → Log cell · every other week → History row · books → Books row, unchanged); retarget `RosterCard.tsx:281-297`. — **✅ BUILT (D12, PR #269)**: `rosterLedgerJumps.ts` gains the inverse direction (`JumpAnchorSlot`, `isJumpAnchorSlot`, `gearRowDomId`, `jumpAnchorSlotOf`, `entryJumpView`); `RosterGearTable` renders the anchors + `.highlight-pulse`; `Roster.tsx` owns the highlight state, the v2-local `gearRowScroll.ts` row scroll, and the 2500 ms clear; `RosterCard.jumpToEntry` resolves the displayed week through `useLogWeek`'s own `resolveLogWeekOverride` and never writes `?week=`; rulings R-D12-A…J in `phase-d-loot-design.md`'s R-18/R-28 build notes; knip unchanged (181/140) | A material jump whose `slotAugmented` is null (universal tomestone) lands on the card, not a row. **Three disclosed residuals:** a genuine week-1-with-no-stored-week tier routes to History rather than the Log (R-D12-C, unit-proven only); the ring one-to-many/one-to-first asymmetry (R-D12-H); Board view renders no anchor at all (R-D12-F cause 4, pre-existing C7/D6a, user-ruled out of scope, pinned by a test) |
...
| **D14** | Elsewhere, shortcuts, write-backs — **✅ BUILT (D14a #272 + D14b #273)** | R-40 `FairnessSummary` → Home (drop the former `Loot.tsx` History mount — `:396-404` at `3922f1af`; removed in #272); R-42's shortcut set with **shell-aware** help/palette groups (§5); §9 `v2-roster-hide-subs`; **write-backs**: D-23, D-27, D-38, D-39, D-40, D-42 (**name D13's two deltas** — mobile collapse deferred to Phase P, "BiS Progress" tile relabelled "Avg BiS progress"; #271), D-43, D-54, D-72, D-37 — and D-18 explicitly **left open** with R-41's blocker restated | Closes the Phase-C closeout's last DoD item |
...
2. **One logging model — asserted by enumeration, not by claim.** ~~Within **v2's Loot subtree**:
   ...
   **Amended at D14b (R-D14-K: the doc is corrected; call sites are never edited to fit it).** The
   literal list undercounted. The test is `loot/loggingModel.enumeration.test.ts`: it scans every
   non-test `.ts`/`.tsx` under `components/loot/**` **and** `components/history/**` (the loot tree mounts
   `history/` modals — `BookLedgerCard.tsx:22-24`, `Loot.tsx:172,189`) for calls to
   `logLootAndUpdateGear`, `logMaterialAndUpdateGear` and the nine book-changing store actions
   (`lootTrackingStore.ts:81-89`). **The test pins every (file, fn) pair** — 35 of them, per-file
   multiplicity included; its reachability tags are documentation, checked only for presence. By function
   (v2-reachable · V1-only · unreachable):
   - `logLootAndUpdateGear` — 3 (`RecipientPicker`, `LogWeekWizard`, `QuickLogWeaponModal` via
     `WeaponPriorityBridge`) · 2 (`QuickLogDropModal`, `history/SectionedLogView`) · 2
   - `logMaterialAndUpdateGear` — 2 (`QuickLogMaterialModal`, `LogWeekWizard`) · 1
     (`history/LogMaterialModal`) · 0
   - `markFloorCleared` — 2 (`BookLedgerCard`, as `MarkFloorClearedModal`'s `onSubmit`; `LogWeekWizard`,
     directly) · 1 · 2
   - `adjustBookBalance` — 1 (`BookLedgerCard`) · 1 · 3
   - `deletePlayerLedger` — 2 (`Loot.tsx`'s D7a resets; `history/PlayerLedgerModal` via `BookLedgerCard`)
     · 1 · 1
   - `clearAllPageLedger` — 1 (`Loot.tsx`, D7a) · 1 · 1; `clearWeekPageLedger`, `clearFloorPageLedger`,
     `clearAllFloorPageLedger`, `clearPlayerWeekPageLedger` — 1 each (`Loot.tsx`, D7a) · 1 each · 0
   ...
   - **Totals: 15 · 11 · 9.** Every V1-only book/ledger pair is in `history/SectionedLogView`.
     ...
     **Amended (DC, 2026-09-25): 35 pairs at D14b; 26 after DC deleted the three unreachable files
     (15 v2-reachable · 11 V1-only), and the `unreachable` tag is retired with them.**
   ...
   *(The tree-wide count is much larger — `QuickLogDropModal`, `QuickLogWeaponModal`,
   ...
   why the assert must be scoped and enumerated, as Phase C's was.)* Still true at D14b: each of those
   six files calls one of the two logging functions.
3. **V1 safety, two-part assert across D0…D14 as a set:** (a) `git diff --stat` over legacy-only paths
   ...
   **mandatory for every slice touching any §2.1 file** — part (a) passes trivially for this phase's
   ...
   exception**, named as such in D0's body.~~ **Two approved exceptions (corrected at D14b):** R-44,
   named as such in D0's body (#223); and PR #242 (`6b41c1aa`, feedback polish) — R-P4's bounded
   initials-centering exception (`plans/2026-08-21-phase-d-feedback-polish.md:29-37`) and its R-V2
   `MembersPanel` swap, which #242's body (:46-55) discloses as changing screen-reader output and the
   sizing basis in both shells.
...
8. Mobile deferred to Phase P by standing ruling — recorded, not silently skipped. Recorded at D14b:
   D13's Team Summary mobile collapse (R-D13-E, #271) is among the deferrals, carried in
   `ROLLOUT_ROADMAP.md` §7b.
...
| **How the `Shift+?` help and command palette stay truthful for two shells** (B4) | D14 — **explicit decision required, no silent default** | **Shell-aware groups** (V1's rows stay, v2's reflect the new set) — so no V1 copy change and the entry stays internal — **✅ Decided at D14: user ruling R-D14-A (2026-09-25), shipped in D14a (#272); V1's help unchanged (literal-fixture tests), release note internal (2.1.30)** |
| **D13's Team Summary reference:** `TeamSummaryEnhanced` vs plain `TeamSummary`, and whether `SummaryPanel` dies with it | D13 slice open | `TeamSummaryEnhanced` (it is what V1 actually renders); `SummaryPanel` deleted once its mount is superseded. **✅ Default taken by D13 (#271), R-D13-A/I** |
...
| `EntryPopover` / `LootFairnessLegend`: import the frozen leaf or fork? | D6 row, not build time | Import — but note `WeeklyLootGrid` uses `FLOOR_COLORS[...].hex` (`:515,522,524`), so DoD 5's grep must scope to v2-authored files. **Ruled 2026-08-23 (R-D6a, D6a build):** `EntryPopover` is forked, not imported — the `×N` route is the v2-owned `LogCellEntriesMenu` (see `phase-d-loot-design.md` R-18 build note for the three superseding legs). `LootFairnessLegend`'s import default stands, unchanged, landing in D6b alongside the count bar (R-D6n) |
| v2 Log grid read-only cells stay inert; legacy grants viewers copy/jump/menu on entry presence alone (`WeeklyLootGrid.tsx:584-609,621`); viewer copy/jump is a possible follow-up slice | ruled D6a (D6-l) | recorded divergence, V1 untouched |
| Priority sub-view persistence: localStorage or the backend user setting? | D1 | localStorage, v2-namespaced |
| **R-21's browser demonstration is deferred at D8** — the modal's edit mode (and its old-vs-new reconciliation matrix) ships unit-proven only (component + coordination tests); D8 has no real week/history context to click it through live | D8 Task 7 | Demonstrate in the running app — the edit door plus the reconciliation matrix — as a named DoD item on **whichever of D5 or D11 lands first** (both mount the material modal's `editEntry` door for the first time: D5's grid material cells, D11's History kebab) — **✅ Discharged in D5 (2026-08-22):** demonstrated live in the running app — grid material cell (Glaze, M10S, week 3, displayed week ≠ clock week 6) → edit door seeded on first paint → recipient switches showed truthful reconciliation ("· Healer One keeps their L. Ring marked as augmented"; gear section suppressed for ineligible recipients) → material changed Glaze→Universal Tomestone + recipient→Healer Two → Save → `PUT /material-log/24 [200]` + refetch chain → grid re-rendered (Glaze cell emptied, "Edit Tome for Healer Two" appeared). The §5-owed F-3 manual case (switch recipient between two glaze-eligible players) was also probed: an away-and-back recipient round-trip lands the slot `Select` on placeholder with the preview honestly disclosing "− Un-mark L. Ring as augmented" (the pre-existing `ui/Select` race below — disclosed, not silent); only one glaze-eligible player existed in dev data, so the true two-eligible-players variant was unreachable without gear surgery — recorded honestly rather than claimed. Evidence: D5 PR screenshots (03–05) + SDD ledger `.superpowers/sdd/2026-08-22-phase-d5-grid-chassis/progress.md` (BROWSER VALIDATION entry) |
| **R-21's edit door carries three mount obligations for whichever of D5/D11 lands first** (beside the deferred-demo row above) — (a) **full roster:** the mounting slice must pass the edit door `allPlayers`, not `mainRosterPlayers`; R-a's "Include substitutes" checkbox has nothing to widen from otherwise, and the R-26 build note only promises the full roster to the free-form door, not edit; (b) **referential stability:** the `allPlayers` array passed must be **stable** across renders — the edit reset effect keys on its identity, so a new array each render re-clobbers in-progress edits. PR #236 round 16 sharpened this: per-render stability is NOT sufficient, because `tier?.players` turns over identity on any background `fetchTier`/`updatePlayer` while the modal is open, and the reset effect then re-applies the entry over the user's in-progress edits (typed note, changed week, picked material, manual slot — silent snap-back, no error). The robust fix belongs in the modal itself when the door mounts: make the edit rehydration run once per open (previous-`isOpen` ref, or keying the modal instance on `editEntry.id`), with a regression test that rerenders with a new `allPlayers` identity after typing a note; (c) ~~copy fix: the UT preview's noun/verb pairing~~ **resolved in D8's own review loop** ("keeps their {noun} marked as {verb}" — PR #236 round 4); (d) **first-paint initializers:** edit mode currently renders one pre-effect frame with free-form defaults (floor/material pills, week, method) before the open-reset effect applies the entry — harmless while nothing mounts the door, visible once something does; fix = lazy `useState` initializers branching on `mode === 'edit'`, the exact pattern `recipientPlayerId`/`selectedSlot` already use (D8 final-review Minor + PR #236 Copilot round 6). Round 7 adds a free-form sibling to this same persistent-mount family: the auto-recipient effect now early-returns when the auto-pick doesn't move (so a subs toggle can't clobber a manual slot pick — PR #236 round 7), which means a consumer that keeps the modal MOUNTED across opens can't rely on that effect to re-derive gear state on reopen; today's doors conditionally mount (fresh lazy seeds per open), and any future persistent-mount consumer must reset/re-derive gear state on open explicitly | D8 final review (F-5) | The mounting slice (D5 or D11) states (a), (b) and (d) as explicit line items in its plan/PR body — not silently inherited from the modal's existing contract — **✅ (a)/(b)/(d) shipped in D5 (commit `8f9a1146`):** (a) full-roster `allPlayers={players}` wired on the edit-door mount in `Loot.tsx`; (b) once-per-open rehydration (`seededForRef` keyed on the `isOpen` edge + `editEntry.id`) plus an identity-churn regression test (rerender with new `allPlayers`/`editEntry` identities after typing a note); (d) lazy edit-mode `useState` initializers — no first-frame free-form flash, proven by a first-render props-recording test. D11 inherits a satisfied contract |
| **The `notes: null` backend contract** — the PUT silently ignores a literal `null` for `notes`, the same wire quirk `MaterialLogEntryUpdate.slotAugmented`'s comment already documents for that field (`types/index.ts:1333-1338`, `loot_tracking.py:1496-1514`). v2's edit path is unaffected (it always sends `''` to clear, per the deltas ledger); legacy V1's edit path sends `null` and so has a live no-op "clear notes" bug. Previously named only in a PR body (Task 5/6 review rounds) — recorded here so it isn't lost | D8 Task 7 | No fix this phase — a named, tracked follow-up; V1's bug stays untouched (deltas ledger) — **✅ CLOSED: already fixed by #239** (`loot_tracking.py:449` loot-log, `:1518` material-log — `model_fields_set`); regression tests `test_loot_tracking.py:427-520`. Doc-only closure, R-DC-G |
| **Phase DoD 2's call-site enumeration test doesn't exist yet** — DoD item 2 requires "a test asserts that exact set" (the `logLootAndUpdateGear`/`logMaterialAndUpdateGear`/books enumeration), but through D8 no such test has been written | D8 Task 7 | **D14** writes it, alongside its other already-assigned write-backs — **✅ Written: `loot/loggingModel.enumeration.test.ts` (D14 Task 4, `5d655c4c`; widened to `history/**` in `12dadaf6`) — see DoD 2** |
| **Radix Select effect-ordering race in the material modal (PRE-EXISTING + V1-LIVE, surfaced by D8's characterization)** — changing the recipient can clobber the slot `Select` back to placeholder: the hidden native-`<select>` bubble-input's value-sync effect races `onNativeOptionAdd`'s native-option re-render, so the `.value` assignment no-ops against the stale (pre-swap) option set and the dispatched change event's value is `''`. The unguarded `onChange` handlers (`QuickLogMaterialModal.tsx:734,760,783`) let that `''` clobber the just-computed slot, so submit sends `slotToAugment: undefined` and the gear augmentation silently skips. Root-cause trace: `QuickLogMaterialModal.test.tsx:311-367`. v2's free-form/edit doors inherit it — D8's lazy seed removed only the mount-time instance | D8 Task 1 characterization; recorded here D8 final review (F-3) | Follow-up ticket against `ui/Select.tsx` (fix applies to both shells — V1 and v2 share the component); no fix this phase. Manual browser-pass case owed: switch recipient between two players who BOTH have eligible slots — **observed in D5's F-3 browser pass (2026-08-22):** an away-and-back recipient round-trip lands the slot `Select` on placeholder with the preview honestly disclosing "− Un-mark L. Ring as augmented"; only one glaze-eligible player existed in D5's dev data, so the two-eligible-players variant stayed unreachable without gear surgery — still open, no fix this phase — **✅ CLOSED (DC).** The Radix race itself was already fixed by #239 (`ui/Select.tsx:147-160` drops the phantom `''`). The residual this row actually named was a different bug — `QuickLogMaterialModal`'s edit door re-deriving the slot on every recipient/material change instead of consulting the entry — fixed in DC Task 3 and demonstrated live with two Twine/Glaze-eligible players (away-and-back restores the recorded slot, no gear line) |
| **`eslint.config.js:13-15`'s `a11yRecommendedWarn` mapping flips upstream-OFF `jsx-a11y` rules to `warn` and strips their options** — surfaced as the root cause of a false-positive warning on D5's grid corner `<td>` (**`LogWeekGrid.tsx:625`** — citation corrected 2026-09-22 at D11 plan-vet, D-REC-3; the old `:246` pointed at `GridCellProps.altHeld`, and `:586` is a *different* rule, `no-static-element-interactions`, on the floor-header div. A one-line targeted `eslint-disable` with reason was used locally instead of touching the shared config). Decided during D5's build, not fixed there. ⚠ **D11 addendum:** this mapping does **not** reach an interactive `<tr>` — `jsx-a11y` treats role `row` as interactive, so neither `no-noninteractive-element-to-interactive-role` nor `no-noninteractive-tabindex` fires on `<tr role="button" tabIndex={0}>` (measured). And because `reportUnusedDisableDirectives` sits at ESLint 9's `"warn"` default here, **a pre-emptive disable costs a warning of its own** — predict nothing, measure first | D5 Task 4 (2026-08-22) | Phase-level follow-up: fix the mapping in a dedicated slice/chore, not silently patched per call site — **✅ CLOSED (DC Task 2).** Lint warnings 903 → 812 across DC (Task 1's deletions −10, the mapping −81; 812 is the new ceiling), no jsx-a11y rule rose; the two v2 directives the fix freed (`LogWeekGrid.tsx:593,632`) were deleted |

**Carried out of Phase D (D14b):** the rows above that were still open at the D14b close (all three closed by DC #274 — see the outcome note below) — the `notes: null` V1 bug, the
`ui/Select` race (with its owed two-eligible-needers browser case) and the `a11yRecommendedWarn`
mapping — plus the D12 row's three disclosed residuals are listed under "Carried out of Phase D" in
`ROLLOUT_ROADMAP.md` §7, so they outlive this plan.

**Outcome (DC, 2026-09-25):** the `notes: null` row above was already fixed by #239 (doc-only
closure); the `ui/Select` race row's Radix race was also already fixed by #239, and the live
residual it actually named (`QuickLogMaterialModal`'s edit door) was fixed in DC Task 3; the eslint
row was fixed in DC Task 2; R-D12-C was fixed in DC Task 4; R-D12-H and R-D12-F cause 4 close as
accepted, pinned residuals (R-DC-F); R-D12-F cause 5 is re-carried, still open. See
`ROLLOUT_ROADMAP.md` §7 for the per-entry evidence.
