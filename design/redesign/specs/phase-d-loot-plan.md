...
| **D9b** | Separators + states | R-29's week separators **only under week sort**, carrying the current-week marker and its measured `text-accent-hover` token; R-34's stats count, filtered-vs-empty (the `?entry=`/`?entryType=` highlight already survived D9a on `historyRowDomId`; D9b's job is the test under separators) | That highlight is **live v2 behaviour** and the destination of shipped C7 roster jumps (`RosterCard.tsx:281-297`) — it must keep resolving against the **unfiltered** logs (`Loot.tsx:39-43`), with a test |
