...
| **`eslint.config.js:13-15`'s `a11yRecommendedWarn` mapping flips upstream-OFF `jsx-a11y` rules to `warn` and strips their options** — surfaced as the root cause of a false-positive warning on D5's grid corner `<td>` (`LogWeekGrid.tsx:246`, a one-line targeted `eslint-disable` with reason was used locally instead of touching the shared config). Decided during D5's build, not fixed there | D5 Task 4 (2026-08-22) | Phase-level follow-up: fix the mapping in a dedicated slice/chore, not silently patched per call site |
