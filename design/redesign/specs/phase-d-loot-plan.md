...
| **R-D12-I** | `pages/GroupViewContent.tsx` | **YES, and on every route with a `?player=` deep link** — its `?player=` effect (`:242-270`) strips both `player` and `slot` on the same 2500 ms timer (`:260,265`), and the effect runs in both shells' mount tree. **Added 2026-09-23 (D12 plan-vet, D-REC-4)**: the one-line `params.delete('slot')` addition necessarily touches it and the inventory omitted it, exactly as it omitted `layout/Layout.tsx` until D11's plan-vet added it above as D-REC-2 | One added line, `params.delete('slot')` (`:265`), inside the existing `?player=` strip. V1 never writes `?slot=`, so the delete is a no-op on every legacy path. ⚠ **This file is not a legacy-only path, so DoD 3 part (a) passes trivially for it** — the guard is Task 4 Step 1b's test in `GroupViewContent.test.tsx` (net-new coverage of this effect), never the `git diff` |
