...
| **D8** | Material modal | R-26 (floor + material selectors, notes) + R-21 (edit mode, old-vs-new augmentation reconciliation) | **Highest freeze risk in the phase.** Every new input optional and off by default so V1's fixed-`floor`/`material` call site (`LootPriorityPanel.tsx:764-767`) renders byte-identically |
