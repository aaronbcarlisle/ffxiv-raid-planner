...
- **Props:** `{ title?: string; icon?: ReactNode; headerRight?: ReactNode; children: ReactNode; className?: string; as?: 'section'|'div'; level?: 'h2'|'h3' }`. Defaults `as='section'`, `level='h2'` (E2a, R-E2-A — every current caller passing a title sits directly under the page `<h1>`, so the default keeps the outline unbroken); pass `level="h3"` for a titled `CardShell` nested inside another titled section. No caller passes `level="h3"` today. (`SessionRsvpCard`'s session-title `<h3>` is its own body markup under its `CardShell`'s default `h2`, not a `CardShell` title.) Use `'div'` when the card is nested inside another landmark (e.g. inside a `<section>`).
...
- **Anatomy (inline variant, drift reconciled E2b/R-E2-E):** 32px role-ringed avatar (`border-2`, `border-color: var(--color-role-*)` via inline style, `gap-3` to the text zone) wrapping `SafeAvatar` (with initials fallback) + `JobIcon` badge sitting on the avatar's **outer edge** (`-bottom-2 -right-2.5`, `rounded-full bg-surface-card ring-2 ring-surface-card`, cut out of the role ring, beyond the initials so neither is covered) + text zone: `text-sm font-medium` name (optionally followed by the `nameAdornment` slot, which never shrinks — the name truncates first) + optional `text-xs text-text-tertiary` subtitle (auto-generated as `"job · position"` when not supplied).
- **Props:** `{ name: string; job?: string; role?: Role; position?: string; subtitle?: ReactNode; avatarUrl?: string; variant?: 'inline'|'board-cell'|'rsvp-row'; nameAdornment?: ReactNode; nameTitle?: string }`. `Role = 'tank'|'healer'|'melee'|'ranged'|'caster'`. `nameAdornment`/`nameTitle` are inline-variant only (E2b, R-E2-D/E): `nameAdornment` is the one slot on the name line — a non-shrinking node after the name (e.g. RosterCard's seat chip) — and `nameTitle` is the name's native hover text (the untruncated name, plus any hint); the name's text node always carries the full name, so truncation is visual only.
...
- **Summary column (drift reconciled, E2b, R-E2-F):** the displayed number and its color come from `utils/playerBisProgress(player)` — the **same** fraction RosterCard's own progress line shows for that player, so the Board and Cards can never disagree. The "No BiS imported" warning row and the muted `—` summary cell keep their own, separate gate: `bisSlotTotals(...).total === 0` (`utils/rosterReadiness.ts`) — that gate's semantics are unchanged and untouched by this reconciliation (#8, Phase F owns them).
...
- **Anatomy (header rewritten E2b, R-E2-D "option 2b" — one line in every state):** a `CardShell` (`as="div"`) wrapped in a positioning `div` that carries the role-colored 3px accent left edge and (when `reorderMode`) the drag-handle attributes/listeners. Header row: `PlayerIdentity` (or an inline rename `Input` when editing) carrying the header's **one seat chip** as its `nameAdornment` — `TankSeatSelector` for a tank (§3.30a), the unchanged `PositionSelector` for everyone else — plus a right-aligned iLvl readout and a kebab `IconButton` (`MoreVertical`); only the name truncates, the chip/iLvl/kebab never shrink or wrap. SUB and a weapon-priority `+N` `Tag` moved off the header onto the subtitle, before the job text; the job swap (formerly an inline `IconButton` opening a `JobPicker` popover) moved into the kebab menu's "Change Job" item. Body: a `ProgressBar` BiS-progress line + a read-only strip of `GearStatusCircle` pips (one per gear slot, `disabled`, no-op `onChange`). Footer: a Lodestone sync-status dot + label, and a right-aligned status CTA (`renderStatus`: Unclaimed+Assign · No BiS+Import · "BiS set" · "needs N").
...
- **Usage rules:** NEW v2 code — does **not** modify or extend the legacy `PlayerCard`. The gear pip strip is read-only/display-only by design: gear **editing** is `GearBoard`'s job (§3.29), not this card's — hence the `disabled` `GearStatusCircle` with a no-op `onChange` rather than mirroring `PlayerCard.handleGearChange`. **Disclosed debt (partly resolved E2b):** a tank's seat now goes through the V2-native `TankSeatSelector` (§3.30a), not the legacy `TankRoleSelector`+`PositionSelector` pair — but every other position (non-tank) still goes through the legacy `player/PositionSelector`, which predates the redesign and has no v2-native replacement yet; a known carry-over, not an oversight, to revisit (or explicitly re-contract as a kept atom) when Roster's remaining legacy imports are audited.
- **Gear-row anchors (D12, `RosterGearTable`):** in expanded density (`RosterCards`' own `density` prop, `components/roster/RosterCards.tsx:117-121`, the C1/D-01 axis), the mounted `RosterGearTable` renders `id="gear-row-{playerId}-{slot}"` (`gearRowDomId`, `components/roster/rosterLedgerJumps.ts`) on every main gear row, plus its own `gear-row-{playerId}-tome_weapon` id on the tome sub-row — the same shape legacy's frozen `GearTable.tsx:325,664` used, net-new here. A `highlightedSlot?: JumpAnchorSlot | null` prop (`JumpAnchorSlot = GearSlot | 'tome_weapon'`) applies the `.highlight-pulse` class (§3.36's same ring treatment) to whichever row matches it. `Roster.tsx` is the only owner of that prop's value: it resolves `?player=`/`?slot=` on mount, drives the row into view via `components/roster/gearRowScroll.ts`, and clears the highlight (both the card's and the row's) on its own 2500 ms timer — mirroring, but never sharing code with, `GroupViewContent`'s identical-cadence `?player=` clear. `RosterGearTable` itself owns no timer and no URL read; `playerId`/`highlightedSlot` are both optional so the table still renders standalone (and in tests) where a global DOM id would be ambiguous.

### 3.30a TankSeatSelector — E2b (R-E2-D)

- **Anatomy:** one trigger `Button` showing both halves in one chip (`"MT · T1"`; an unset half reads `"--"`) opening one `Popover` with a labelled row per half (tank role, raid position) — each row's options render with `PositionSelector`'s selected-fill treatment (`POSITION_SELECTED` per letter) and a per-row Clear that only renders while its half is set.
- **Props:** `{ tankRole: TankRole | null | undefined; position: RaidPosition | null | undefined; onTankRoleSelect: (role: TankRole | undefined) => void; onPositionSelect: (position: RaidPosition | undefined) => void; player: SnapshotPlayer; userRole?: MemberRole | null; currentUserId?: string; isAdmin?: boolean }`. The two callbacks are exactly `TankRoleSelector`'s `onSelect` and `PositionSelector`'s `onSelect`.
- **States:** popover closed/open. Deliberate deltas from the two selectors it replaces: a pick does **not** close the popover (the halves are usually set together — outside click/Escape close it); opening focuses the selected (or first) tank-role option.
- **Usage rules:** V2-only, RosterCard's tank seat chip only (§3.30). `TankRoleSelector` and `PositionSelector` themselves are untouched — legacy's `PlayerCardHeader` still renders them, and RosterCard still renders bare `PositionSelector` for non-tanks.
