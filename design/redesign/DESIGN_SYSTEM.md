...
- **Density:** accent color-mix steps over `surface-card` — **8/16/28/45/70%** — selected by the count/total ratio (`<25%→8, <50%→16, <75%→28, <100%→45, 100%→70`), via `color-mix(in srgb, var(--color-accent) N%, var(--color-surface-card))`. Same formula as the F1/F6d gear-board density steps — no new tokens introduced.
...
- **States:** empty (no recommendations) | canManage row (`Button variant="ghost"` wrapping the whole row, `aria-label="Propose session {when}"`) | view-only row (plain `div`, non-interactive) | scheduled row (id matches `scheduledSlotIds` → appends `" · scheduled"` as text, not color-only).
