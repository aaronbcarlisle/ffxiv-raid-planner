...
import { ViewModeToggle, SortModeSelector, GroupViewToggle, MobileBottomNav, Checkbox, Modal, PageSkeleton } from '../components/ui';
...
export function GroupViewContent({ slots, actions, onSwitchToClassicUi }: GroupViewContentProps) {
  ...
  const {
    ...
    clipboardPlayer,
    setClipboardPlayer,
    // setShowKeyboardHelp is deliberately NOT destructured (Phase R): Shift+? is
    // handled globally by useGlobalKeyboardShortcuts (Layout owns the modal), so
    // the keyboard hook no longer takes the setter. showKeyboardHelp itself stays
    // in the isAnyModalOpen composition below to minimize drift from f45a241
    // (the restored state field exists and is permanently false).
    showKeyboardHelp,
    showLogLootModal,
    setShowLogLootModal,
  ...
  useGroupViewKeyboardShortcuts({
    ...
    setEditingPlayerId,
    setHighlightedPlayerId,
    // Phase R (dual shell): the legacy gear-tab surface travels via this param
    // ONLY when the legacy gear body can render (no v2 gear slot). Its presence
    // gates the legacy Alt sub-tab/quick-action bindings inside the hook — with
    // a v2 gear slot those modals/sub-tabs have no renderer, so the bindings
    // must not register (see useGroupViewKeyboardShortcuts).
