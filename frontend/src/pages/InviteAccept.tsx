...
export function InviteAccept() {
  ...
  const ROLE_LABELS: Record<MemberRole, string> = {
    ...
    lead: t('invite.roleLead'),
    member: t('invite.roleMember'),
    viewer: t('invite.roleViewer'),
  };

  const ROLE_DESCRIPTIONS: Record<MemberRole, string> = {
    owner: t('invite.roleOwnerDesc'),
    lead: t('invite.roleLeadDesc'),
    member: t('invite.roleMemberDesc'),
    viewer: t('invite.roleViewerDesc'),
  };
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-surface-base flex items-center justify-center">
        <div className="text-text-secondary">{t('invite.loading')}</div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-surface-base flex items-center justify-center p-4">
        <div className="bg-surface-card border border-border-default rounded-lg p-8 max-w-md w-full text-center">
          ...
          <h1 className="text-xl font-display text-text-primary mb-2">{t('invite.notFoundHeading')}</h1>
          <p className="text-text-secondary mb-6">
            {error || t('invite.notFoundDesc')}
          </p>
          <Link
            to="/"
      ...
      <div className="bg-surface-card border border-border-default rounded-lg p-8 max-w-md w-full">
        ...
        <div className="bg-surface-elevated border border-border-default rounded-lg p-4 mb-6">
          ...
          <div className="flex items-center gap-2 text-sm">
            <span className="text-text-secondary">{t('invite.roleLabel')}</span>
        ...
        {!user ? (
          ...
          <div className="space-y-3">
            <Button
              ...
              className="w-full"
            >
              {isAccepting ? t('invite.acceptingButton') : t('invite.acceptButton')}
            </Button>
