 ...
 * pinned pageMode, via the shared `./newShellTestScaffold` builders). `Loot` is
 * stubbed — the point is the slot WIRING, not Loot's internals (covered by its
 * own tests). The legacy gear body leaf (GearSyncDashboard, mounts at
 * gearSubTab='sync' in the RED run) is stubbed.
 */
...
vi.mock('../hooks/useGroupViewState', async () => {
  ...
  return { useGroupViewState: () => makeGroupViewStateMock({ pageMode: 'gear' }) };
});
