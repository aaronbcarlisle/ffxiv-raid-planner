...
export function RevertWeekSummaryModal({
  ...
  const countsLine = [
    countLabel(weekLoot.length, 'drop'),
    countLabel(weekMaterials.length, 'material'),
    countLabel(weekLedger.length, 'book entry', 'book entries'),
  ].join(' · ');
            ...
            <div className="max-h-64 space-y-3 overflow-y-auto">
              {weekLoot.length > 0 && (
                <div>
                  ...
                  <ul className="space-y-0.5 pl-4 text-xs text-text-secondary">
                    {weekLoot.map((entry) => (
                ...
                <div>
                  ...
                  <ul className="space-y-0.5 pl-4 text-xs text-text-secondary">
                    {weekLedger.map((entry) => (
                      <li key={entry.id} className="list-disc">
                        {`Page ${entry.bookType} (${entry.transactionType === 'earned' ? '+' : ''}${entry.quantity})`}
                        {' → '}
