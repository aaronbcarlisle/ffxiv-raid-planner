...
export function InlinePlayerEdit({ player, userRole, onSave, onCancel }: InlinePlayerEditProps) {
  ...
  // Role filtering state (for non-template slots only)
  ...
  // Filter jobs based on selected roles (for non-template slots)
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-4">
          <input
            ...
            className={`w-full bg-surface-elevated border rounded px-3 py-2 text-text-primary placeholder:text-text-muted focus:outline-none transition-colors ${
              showNameError
        ...
        {/* Job selection - use JobPicker for template slots, custom picker for non-template */}
        {player.templateRole ? (
          <div className="mb-4">
            <JobPicker
              selectedJob={job}
          ...
          <>
            ...
            {/* Job picker dropdown (non-template slots only) */}
            <div className="mb-4 relative" ref={jobPickerRef}>
              <button
                type="button"
                onClick={() => setIsJobPickerOpen(!isJobPickerOpen)}
                className="w-full bg-surface-elevated border border-border-default rounded px-3 py-2 text-left flex items-center gap-3 focus-visible:border-accent focus:outline-none hover:border-text-muted"
              >
                {selectedJobInfo ? (
                  <>
                ...
                <div className="absolute z-50 mt-1 left-0 right-0 bg-surface-raised border border-border-default rounded-lg shadow-lg">
                  ...
                  <div className="p-2 border-b border-border-default">
                    <input
                      ...
                      className="w-full bg-surface-elevated border border-border-default rounded px-3 py-1.5 text-sm text-text-primary placeholder:text-text-muted focus-visible:border-accent focus:outline-none"
                    />
                  </div>
