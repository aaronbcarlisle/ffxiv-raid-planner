/**
 * ScheduleIntegrationsPanel
 *
 * The Discord (reminders + guild events) and private iCal integrations block,
 * extracted VERBATIM from `ScheduleTab` (promote-and-repoint). It owns its own
 * `useScheduleStore()` subscription, its ~21 local states, the integration
 * handlers, the settings-sync effect, and the Discord-mirror polling effect, so
 * it renders identically whether mounted from the legacy Schedule tab or from
...
import type { DiscordMirrorStatus, MemberRole } from '../../types';
...
interface ScheduleIntegrationsPanelProps {
  groupId: string;
  canManage: boolean;
  userRole: MemberRole | null | undefined;
}

export function ScheduleIntegrationsPanel({ groupId, canManage, userRole }: ScheduleIntegrationsPanelProps) {
  const {
    sessions,
    settings,
    isLoadingSettings,
