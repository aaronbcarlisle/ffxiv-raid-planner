...
export const RELEASES: Release[] = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        category: 'fix',
        title: 'The D11 mutation harness can no longer report a crashed run as healthy',
        description:
  ...
  },
  {
    ...
    title: 'Phase D slice D11 — History row affordances (v2 preview)',
    items: [
      {
        category: 'improvement',
        title: 'History rows are clickable — and only say so when they are',
        description:
          'V2 preview: a History row now opens its entry for editing when you click it, with Shift+Click to copy a link to that entry and Alt+Click to jump to the player who received it. Material rows can be edited from History for the first time. The row kebab — also reachable by right-clicking the row — carries Edit, Copy link, Jump to the player, "View week N in Log", and Delete; that last jump takes you to the Log tab on that week with the entry highlighted. Ctrl+Shift+F focuses the History search box, and it now appears in the Shift+? shortcuts list. The row shows a clickable cursor only when clicking it would actually do something — when your permissions let you edit, or while you hold Alt over a row whose player you can jump to — so a read-only view no longer advertises an action that would do nothing, and row text stays selectable for copying.',
        pr: 266,
        prTitle:
          'feat(v2): D11 History row affordances — the row becomes a control, and only where it can act',
      },
    ],
    internal: true,
  },
    ...
    items: [
      {
        ...
        description:
          'V2 preview: History’s Week/Player/Source dropdowns are replaced by a single search box with a structured syntax — player:"Tank One", floor:m9s,m10s, type:gear, source:tome, week:3. The Type, Floor and Player pills above it are now shortcuts into that box: clicking one writes its token, clicking it again removes it, so there is one place to look when the table comes back empty instead of two. Picking two floors returns both rather than nothing, "source:tome" gives the tome-or-purchase grouping the old Source dropdown had but no typed filter could reach, and a half-typed filter like "player:" no longer empties the table while you are still typing the name. An unrecognised filter now says so instead of silently doing nothing. The search stays on your screen only — it is never added to a copied entry link, so a shared link can never arrive filtered.',
        pr: 265,
        prTitle:
    ...
    items: [
      {
        ...
        description:
          "V2 preview: click any of its seven sortable columns to reorder the list (the eighth is the row's actions menu), newest first by default. History now shows floor chips, slot icons, the weapon's job on weapon rows, and the augmented slot on material rows. The header row stays put while you scroll, and each row's actions menu announces its own slot and recipient to screen readers instead of a shared \"Entry actions\". Week separators, search, and clicking a row to edit come in the next slices.",
        pr: 262,
        prTitle:
