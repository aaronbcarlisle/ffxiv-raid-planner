    ...
    items: [
      {
        ...
        description:
          'V2 preview: a History row now opens its entry for editing when you click it, with Shift+Click to copy a link to that entry and Alt+Click to jump to the player who received it. Material rows can be edited from History for the first time. The row kebab — also reachable by right-clicking the row — carries Edit, Copy link, Jump to the player, "View week N in Log", and Delete; that last jump takes you to the Log tab on that week with the entry highlighted. Ctrl+Shift+F focuses the History search box, and it now appears in the Shift+? shortcuts list. The row only shows a clickable cursor when your permissions actually let you edit, so a read-only view no longer advertises an action that would do nothing, and row text stays selectable for copying.',
        pr: 266,
        prTitle:
